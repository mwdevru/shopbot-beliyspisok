...
### API
- Обновлён модуль mwshark_api.py согласно новой документации API
- Добавлена поддержка параметра `devices` при создании/продлении подписки
- Добавлен метод `change_subscription_devices` для изменения лимита устройств

### Исправления
- Исправлена критическая ошибка `ValueError: invalid literal for int() with base 10: 'None'` в обработчике show_key_handler
- Добавлена защита от None key_id во всех обработчиках (show_key, show_qr, howto_vless, extend_key)
- Исправлено отображение ключей с key_id=None в веб-панели (user_detail.html)
- Исправлена клавиатура управления ключами - ключи без ID теперь отображаются корректно
